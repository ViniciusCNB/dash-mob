...
import { useDashboardLinha } from "@/hooks/useApiQueries";
import { FilterIcon, MapPin } from "lucide-react";
import { useEffect } from "react";

const LinhaIndividual = () => {
  const { appliedLinhas, selectedLinhas, applyFilters } = useFilter();
  const linhaSelecionada = appliedLinhas.length > 0 ? appliedLinhas[0] : null;

  // Aplicar filtros automaticamente quando uma linha é selecionada
  useEffect(() => {
    if (selectedLinhas.length > 0 && appliedLinhas.length === 0) {
      applyFilters();
    }
  }, [selectedLinhas, appliedLinhas, applyFilters]);

  // Buscar dados do dashboard da linha selecionada (inclui pontos do mapa)
  const { data: dashboardData, isLoading: dashboardLoading, error: dashboardError } = useDashboardLinha(linhaSelecionada?.id_linha || 0);
          ...
          <CardContent className="p-8 text-center">
            <div className="space-y-4">
              <div className="text-muted-foreground">
                ...
                <h3 className="text-lg font-medium">Selecione uma linha para análise</h3>
                <p className="text-sm">
                  Use o filtro acima para escolher uma linha específica e visualizar suas informações detalhadas
                </p>
                {selectedLinhas.length > 0 && (
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800">
                      Linha selecionada: {selectedLinhas[0].cod_linha}
                      {selectedLinhas[0].nome_linha && ` - ${selectedLinhas[0].nome_linha}`}
                    </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <h1 className="text-2xl font-bold">
          Linha {linhaSelecionada.cod_linha}
          {linhaSelecionada.nome_linha && ` - ${linhaSelecionada.nome_linha}`}
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg font-semibold text-center">Informações da Linha</CardTitle>
          </CardHeader>
              ...
              {dashboardLoading ? (
                ...
                <div className="w-full h-full">
                  <Barplot 
                    ...
                    chartTitle={`Passageiros por dia da semana - Linha ${linhaSelecionada.cod_linha}`}
        ...
        {/* Gráfico de Justificativas das Ocorrências */}
        <Card>
          <CardHeader>
            <CardTitle className="text-lg font-semibold text-center">Justificativas das ocorrências</CardTitle>
          </CardHeader>
          <CardContent className="p-4 flex items-center justify-center">
            <div className="h-72 w-full">
              {dashboardLoading ? (
      ...
      <Card className="p-0">
        <CardContent className="p-0">
          <div className="h-[600px]">
            ...
            ) : (dashboardData?.mapa_pontos?.features && dashboardData.mapa_pontos.features.length > 0) || 
                (dashboardData?.mapa_bairros?.features && dashboardData.mapa_bairros.features.length > 0) ? (
              <LeafletPointsMap
                pontosData={dashboardData.mapa_pontos}
                bairrosData={dashboardData.mapa_bairros}
                height={600}
                selectedLine={linhaSelecionada.cod_linha}
                onLineSelect={() => {}}
              />
            ) : (
              <div className="flex items-center justify-center h-full text-muted-foreground bg-gray-50 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-sm">Não foram encontrados pontos para esta linha</p>
                </div>
              </div>
            )}
          </div>
...
export default LinhaIndividual; 